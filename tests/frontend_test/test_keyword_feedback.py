...
class KeywordFeedbackTest(unittest.TestCase):
    """Tests for the small state machine that powers keyword review behavior."""
    def setUp(self) -> None:
        # Ordered candidate list used by several tests. Higher-ranked terms come first.
        self.candidates = [
    ...
    def test_initialize_sets_visible_and_selected_terms(self) -> None:
        # New results should show the top N candidates and start with all of them selected.
    ...
    def test_regenerate_replaces_exact_number_removed(self) -> None:
        # If the reviewer keeps only one term, the helper should fill exactly
        # the other two slots from unused candidates.
        result = initialize_image_result("image", self.candidates, target_count=3)
    ...
    def test_regenerate_does_not_reintroduce_rejected_terms(self) -> None:
        # Once the reviewer rejects a term, it should never be offered again
        # during later regeneration rounds for the same image.
        result = initialize_image_result("image", self.candidates, target_count=3)
    ...
    def test_export_labels_only_returns_currently_selected_terms(self) -> None:
        # Export output should mirror the current checkbox state exactly.
        result = initialize_image_result("image", self.candidates, target_count=3)
